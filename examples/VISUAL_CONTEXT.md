...
import { createCinemaVisualContext } from './cinema-visual-context.js';

cinemaPlayer.init({ baseUrl: '/cinema' });

const cinemaVisual = createCinemaVisualContext(cinemaPlayer, {
  baseUrl: '/cinema',
  windowSec: 90,
  rowSec: 30,
  sheetWidth: 1600,
...
async function collectCinemaContext() {
  const cin = await cinemaVisual.collect();
  return {
...
```js
const visual = await cinemaVisual.buildAdaptiveSheet({
...
```js
createCinemaVisualContext(cinemaPlayer, {
  baseUrl: '/cinema',
...
- 让 AI 线性“看完”一部电影：`buildAdaptiveSheet()`，`targetKeyframesPerSheet=12-20`，`maxSheetSec=300-600`。
...
如果片库里有 `Sintel Trailer (2010)`，可以启动 clove-cinema 和静态文件服务后打开：

```bash
python server.py --root ~/cinema --port 8770 --allow-origin http://127.0.0.1:8788
...
如果 clove-cinema 跑在别的端口：

```text
http://127.0.0.1:8788/examples/frontend/visual-test.html?cinema=http://127.0.0.1:8771/cinema
```

## 当前限制

- 这是前端原型：用隐藏 video 离屏 seek 抽帧，不会打断当前播放器，但第一次生成会有等待。
- 依赖浏览器能从 `/cinema/stream/{id}` seek。跨域部署时需要正确 CORS，否则 canvas 会被 taint。
- 还没有音轨 rail。
