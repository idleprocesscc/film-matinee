# 集成指南 — 把 film-matinee 接进你自己的聊天前端
...
后端 `film-matinee` 已经在跑（默认 `127.0.0.1:8770/film-matinee/*`）。
你的前端要能访问 `/film-matinee/list`、`/film-matinee/sync/...`、`/film-matinee/stream/...` 这几个路由
...
```html
<!-- index.html 头里加 -->
<link rel="stylesheet" href="/static/film-matinee-player.css">
```
...
filmMatineePlayer.init({ baseUrl: '/film-matinee' });   // 跨域时填完整 URL
window.filmMatineePlayer = filmMatineePlayer;          // 可选：暴露给其他模块用
```

`baseUrl` 是 film-matinee 的路由前缀。同源用 `/film-matinee`，跨域填
`https://films.your.site/film-matinee` 之类的。
...
在某个路由 / 页面里调 `GET /film-matinee/list` 渲染列表，点开调
`filmMatineePlayer.open(id, title)` 启动播放。
...
async function renderFilmMatineePage(root) {
  const r = await fetch('/film-matinee/list');
  ...
  root.querySelectorAll('.film-card').forEach(card => {
    card.querySelector('button').onclick = () => {
      filmMatineePlayer.open(card.dataset.id, card.querySelector('h3').textContent);
    };
  });
}
```
...
`filmMatineePlayer.open()` 之后浮窗立刻可见，video 元素自己开始拉 `/film-matinee/stream/{id}`
（Range 流，浏览器原生处理 seek、缓冲，不用你管）。

---

## 3. 发消息前 — 采集 film-matinee 上下文

这是核心。每次你的聊天前端要发消息给 AI 时，**在调 chat API 前**先调一次
`collectFilmMatineeContext()`，把当前帧 + 增量字幕拼上。

如果要实验“关键帧 + 色带 + 字幕 sidecar”的 film-matinee sheet，不要改下面这段旧版函数。
直接看 `examples/VISUAL_CONTEXT.md`，用 `film-matinee-visual-context.js` 里的
`filmMatineeVisual.collect()`。它会返回一张 `film-matinee-sheet` 图片和完整字幕 sidecar，
适合需要保留镜头语言的片段。
...
```js
const filmMatineeInjected = {};  // { [filmId]: lastInjectedTs }
```

每次发消息时：
- 算 `lastTs = filmMatineeInjected[id] || 0`，`curTs = status.ts`
- 如果 `curTs > lastTs` → 请求 `[lastTs, curTs]` 区间的字幕
- 更新 `filmMatineeInjected[id] = max(lastTs, curTs)`
...
async function collectFilmMatineeContext() {
  ...
  const curTs = st.ts || 0;
  let lastTs = filmMatineeInjected[st.id] ?? 0;
  if (curTs < lastTs - 30) lastTs = Math.max(0, curTs - 60);

  let subBlock = '';
  if (curTs > lastTs) {
    try {
      const url = `/film-matinee/sync/${encodeURIComponent(st.id)}?from=${lastTs}&to=${curTs}`;
    ...
    } catch (e) { console.warn('[film-matinee] sync failed', e); }
    filmMatineeInjected[st.id] = Math.max(lastTs, curTs);
  }
  ...
  let image = null;
  const shot = filmMatineePlayer.snapshot();
  if (shot) {
...
async function sendMessage(userText) {
  const cin = await collectFilmMatineeContext();
...
`filmMatineePlayer.snapshot()` 返回的完整 dataURL，不用拆。
...
- ❌ **不要把 filmMatineeInjected 持久化到 localStorage**（除非你想过 corner case）。
...
- ❌ **不要让 video 元素的 src 在切换 SPA 路由时被清掉**。
  film-matinee-player.js 把浮窗挂在 body 上就是为了让它跨路由不销毁。
  你的路由器卸载某个房间时，别误把 `<video>` 一起拆了。

- ❌ **不要把 film-matinee 上下文跟其他 context（reading-context、code 等）放在同一个 prefix 段里**。
...
`film-matinee-player.css` 里的颜色 / 圆角 / z-index 全是建议值。
你的 app 有自己的设计语言就改 `.fm-bar` 背景、`.fm-btn` 颜色、
`#film-matinee-player` 的 border-radius 等。JS 那边不用动。

挂位置不喜欢右下角？改 `#film-matinee-player` 的 `right` / `bottom` 默认值。
